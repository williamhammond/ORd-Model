...
ptm <- proc.time()
cores <- detectCores()
#clx <- makePSOCKcluster(cores)
clx <- makeCluster(cores)
registerDoParallel(clx)
#load(file <- "~/Dropbox/classwork/Spring2015/OrdModel/variables.RDat")

results <- matrix(nrow = 1, ncol = 16)
features <- list()
...
res <- foreach(dCount=1:dSize, .combine=rbind)%dopar%
{
  ...
  if (icelltype == 1)
  {
    gNal[dCount] <- gNal[dCount] * 0.6
  }
  if (icelltype !=  0 )
  {
    Gto[dCount] <- Gto[dCount]  *  4.0 
  }
  Aff <- 0.6
  Afs <- 1 - Aff
  ...
  if (icelltype  ==  1)
  {
    PCa[dCount] <- PCa[dCount]  *  1.2
  }else if (icelltype  ==  1){
    PCa[dCount] <- PCa[dCount]  *  2.5 }
  
  PCap   <- 1.1       *  PCa[dCount]
  PCaNa  <- 0.00125   *  PCa[dCount]
  PCaK   <- 3.574e-4  *  PCa[dCount]
  PCaNap <- 0.00125   *  PCap
  PCaKp  <-3.574e-4   *  PCap
  
  if(icelltype  ==  1)
  {
    GKr[dCount] <- GKr[dCount]  *  1.3  
  } else if (icelltype  ==  2){
    GKr[dCount] <- GKr[dCount]  *  0.8  }
  
  if(icelltype  ==  1)
  {
    GKs[dCount] <- GKs[dCount]  *  1.4  
  }
  
  if(icelltype  ==  1)
  {
    GK1[dCount] <- GK1[dCount]  *  1.2 
  } else if(icelltype  ==  2){
    GK1[dCount] <- GK1[dCount]  *  1.3  }
  
  if(icelltype  ==  1)
  {
    Gncx[dCount] <- Gncx[dCount]  *  1.1  
  } else if(icelltype  ==  2)  {
    Gncx[dCount] <- Gncx[dCount] * 1.4  }
  k3p <- 1899.0
  k4p <- 639.0
  
  if(icelltype  ==  1)
  {
    Pnak[dCount] <- Pnak[dCount] * 0.9  
  } else if( icelltype  ==  2 ){
    Pnak[dCount] <- Pnak[dCount] * 0.7 }
...
#     outputFileConn <- file(output)
    ...
    for (ntime in 1:nend)
    {
      ...
      for (i in 1:nx)
      {
        ...
        if(iv1 < 0 || iv1 > nvt)
        {
          cat("Voltage index outside volate table: \n",ntime,ntime*dt,v[i],iv1)
          stop  
        }
        ...
        INaL <- gNal[dCount]*(v[i]-ENa)*xmL[i]*((1.0-fINaLp)*xhL[i] + 
                                          fINaLp*xhLCaMK[i])
        ...
        Ito <- Gto[dCount]*(v[i]-EK)*((1.0-fItop)*xa[i]*xi+fItop*a_CaMK[i]*ip)
        ...
        ICaL <- (1.0-fICaLp)*PCa[dCount]*PhiCaL*xd[i]*(xf*(1.0-xnca[i])+
                                                 xjca[i]*fca*xnca[i])
        ...
        IKr <- GKr[dCount]*sqrt(ko/5.4)*xr*RKr[iv1]*(v[i]-EK)
        ...
        IKs <- GKs[dCount]*KsCa*xs1[i]*xs2[i]*(v[i]-EKs)
        
        # calculate IK1
        
        xk1[i]  <-  Xk1inft[iv1]-(Xk1inft[iv1]-xk1[i])*exptauXk1t[iv1]
        
        IK1 <- GK1[dCount]*sqrt(ko)*RK1[iv1]*xk1[i]*(v[i]-EK)
        ...
        INaCa_i <- 0.8*Gncx[dCount]*allo*(zna*JncxNa+zca*JncxCa)
        ...
        INaCa_ss <- 0.2*Gncx[dCount]*allo*(zna*JncxNa+zca*JncxCa)
        ...
        INaK <- Pnak[dCount]*(zna*JnakNa[i]+zk*JNakK[i])
        ...
        for (iloop in 1:20)
        {
          ...
          tau_rel <- bt[dCount]/(1.0+0.0123/Cajsr[i])
...
#elshrif:Tusscher mme4362$ cd debug
#elshrif:debug mme4362$ ls
#elshrif:debug mme4362$ ./tusscher
